<?php
	...
	$pageData['isGalleryPage']=1;
	
	$pageData["logoTitle"]="Gallery";
	
	if( isset($_GET['album']))
	{
		$pageData["isAlbum"]="yes";
		$i = 0;
		while ($i <= 100) {
		   ...
		   //if the picture url is a small picture, replace the _s with _n to pull the larger version of the image
			...
			if (($_GET['album']) == $pageData['albums']['data'][$i]['id']){
				...
				$pageData["logoTitle"]=$pageData['albumsOnly']['name'];
			}
			$i++;
		}
		$pageData['isalbumdac'] = 1;
		display($pageData);
		//echo "<pre>";print_r($pageData['albumsOnly']);echo "</pre>";
	}
	else
	{
		$pageData['albumsOnly'] = $pageData['albums']['data'];
